...
public class BoundingBox implements Cloneable, ConfigurationSerializable {
    ...
    public BoundingBox expand( BlockFace blockFace, double expansion) {
        ...
        return blockFace == BlockFace.SELF ? this : this.expand(blockFace.getOppositeFace(), expansion);
    }
    ...
    public RayTraceResult rayTrace( Vector start,  Vector direction, double maxDistance) {
        ...
        if (!Double.isFinite(start.getX()) || !Double.isFinite(start.getY()) || !Double.isFinite(start.getZ())) {
            throw new IllegalArgumentException("Start vector contains non-finite values!");
        }

        Validate.notNull(direction, "Direction is null!");

        if (!Double.isFinite(direction.getX()) || !Double.isFinite(direction.getY()) || !Double.isFinite(direction.getZ())) {
            throw new IllegalArgumentException("Direction vector contains non-finite values!");
        }
